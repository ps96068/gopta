...
class ManagerView(ModelView):
    ...
    #     ):
    #         errors["telegram_id"] = "Invalid Telegram ID !"
    #     elif await self._check_telegram_id_data(telegram_id=data["telegram_id"]):
    #         errors["telegram_id"] = "Telegram ID exists!"
    #     if (
    #             not data["username"] or
    #             len(data["username"]) < 4
    ...
    #     ):
    #         errors["email"] = "Invalid email adress !"
    #     elif await self._check_email_data(email=data["email"]):
    #         errors["email"] = "Email adress exists!"
    #     if len(errors) > 0:
    #         raise FormValidationError(errors)
    #
    ...
    async def before_create(
        ...
        print("manager before_create")
        setattr(obj, 'password_hash', get_hashed_password(data["password_hash"]))
        setattr(obj, 'role', StaffRole.manager.value)
